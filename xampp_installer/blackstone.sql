...
CREATE TABLE `reports` (
  ...
  `audited_company_id` int(11) DEFAULT NULL,
...
INSERT INTO `reports` (`id`, `name_doc`, `audited_company_id`, `vulnerabilities`, `state`, `date`, `recommendations`, `proposals`, `conclusions`) VALUES
...
CREATE TABLE `pocs` (
  ...
  `id_scope_vulnerability` int(11) DEFAULT NULL,
...
INSERT INTO `pocs` (`id`, `conclusion`, `description`, `id_scope_vulnerability`, `ordre`) VALUES
...
INSERT INTO `scope` (`id`, `url`, `id_report`, `ordre`) VALUES
...
--
-- Estructura de tabla para la tabla `scope_vulnerabilities`
--

CREATE TABLE `scope_vulnerabilities` (
  `id_vulnerability` int(11) NOT NULL,
  `description` text NOT NULL,
  `level` double(10,1) NOT NULL,
  `id` int(11) NOT NULL,
  `name` text NOT NULL,
...
--
-- Volcado de datos para la tabla `scope_vulnerabilities`
--

INSERT INTO `scope_vulnerabilities` (`id_vulnerability`, `description`, `level`, `id`, `name`, `id_scope`, `solution`) VALUES
...
(1, 'blackstone9', '$2y$10$eWlAFAHveUj4.WGy/2xCuOWXYAaVHe6PU3C8PN/reEdKQU69QRDlC');
...
INSERT INTO `vulnerabilities` (`id`, `description`, `solution`, `level`, `name`) VALUES
...
(166, 'Permite a los atacantes incluir y ejecutar archivos arbitrarios.', 'Evitar el uso de entradas proporcionadas por el usuario en las rutas de inclusión de archivos.', 3, 'vulnerabilities de inclusión de archivos'),
...
(185, 'Permite que los atacantes carguen archivos maliciosos en un servidor', 'Validar el tipo de archivo cargado y usar mecanismos seguros para la carga de archivos', 3, 'vulnerability de carga remota de archivos'),
(186, 'Explota fallas en la lógica de la aplicación para realizar acciones no autorizadas', 'Implementar controles de acceso adecuados y validación en la lógica de negocios', 2, 'vulnerabilities de la lógica de negocios'),
(187, 'Ocurre cuando varios procesos acceden a recursos compartidos simultáneamente', 'Usar bloqueos y mecanismos de sincronización para evitar condiciones de carrera', 3, 'vulnerability de condición de carrera'),
...
(193, 'Se refiere a una vulnerability desconocida y sin parches explotada por atacantes', 'Mantener el sistema actualizado con parches de seguridad y monitorear las comunidades y foros de seguridad en busca de amenazas emergentes', 4, 'vulnerability de día cero'),
...
(197, 'Aprovecha las debilidades en las implementaciones para las connectiones OAuth y OpenID', 'Seguir las mejores prácticas de seguridad para la integración de las connectiones de OAuth y OpenID', 3, 'vulnerabilities en las connectiones de OAuth y OpenID'),
...
(202, 'Permite a los atacantes secuestrar una sesión activa después de que un usuario haya cerrado la sesión', 'Establecer un período de tiempo de espera de sesión corto y correctamente configurado', 2, 'vulnerability de tiempo de espera de sesión'),
...
(215, 'Aprovecha las fallas de seguridad conocidas en los componentes del Directorio Activo', 'Mantener el sistema actualizado con los parches de seguridad y seguir las mejores prácticas para la administración de parches', 3, 'vulnerabilities sin parches'),
...
(221, 'Permite el acceso no autorizado a datos de zona DNS', 'Limitar permisos de transferencia de zona y configurar cortafuegos para bloquear solicitudes no autorizadas', 3, 'vulnerability de transferencia de zona DNS'),
...
(225, 'Provoca incoherencias y daños en los datos entre los Controladores de Dominio', 'Supervisar y mantener el state de la replicación del Directorio Activo y garantizar la conectividad de red entre los Controladores de Dominio', 3, 'Problemas de replicación del Directorio Activo'),
(226, 'Permite a los atacantes conservar los derechos de acceso de dominios anteriores', 'Limitar el uso del historial de del Identificador de Seguridad (SID) y administra cuidadosamente las relaciones de confianza entre bosques', 4, 'vulnerability del historial del identificador de seguridad (del Identificador de Seguridad (SID))'),
...
(235, 'Aprovecha las debilidades en las implementaciones del Servicio de Federación en el Directorio Activo', 'Mantener el sistema actualizado con los parches del Servicio de Federación en el Directorio Activo y seguir las mejores prácticas de seguridad', 3, 'vulnerabilities de los servicios federados del Directorio Activo (AD FS)'),
...
(244, 'Permite a los atacantes redirigir las consultas del DNS a direcciones maliciosas', 'Implementar DNSSEC y monitorizar regularmente el state de la caché del DNS', 4, 'Envenenamiento de caché del DNS'),
...
(250, 'Permite a los atacantes usar tickets de fuerza bruta en Kerberos', 'Habilitar la autenticación previa de Kerberos y aplicar contraseñas seguras', 4, 'vulnerabilities de autenticación previa de Kerberos'),
...
(255, 'Permite a los atacantes abusar de la configuración de GPO para realizar cambios no autorizados', 'Asegurar GPO y hacer cumplir las políticas de contraseñas seguras en cuentas de GPO', 3, 'vulnerabilities de políticas de grupo'),
...
(260, 'Provoca inconsistencias en la replicación entre controladores de dominio', 'Supervisar y mantener el state de replicación del Directorio Activo y resolver los problemas de sincronización', 3, 'Problemas de sincronización de controlador de dominio'),
(261, 'Permite la emisión no autorizada de certificados digitales', 'Asegurar y controlar los servicios de la autoridad de certificación y mantenener unas políticas de certificación sólidas', 4, 'vulnerabilities de la autoridad de certificación'),
...
(264, 'Permite a los atacantes hacerse pasar por otros usuarios o aplicaciones', 'Implementar una seguridad de token adecuada y validar la autenticidad del token', 4, 'vulnerability de suplantación de token'),
...
(266, 'Permite modificaciones no autorizadas de registros DNS', 'Restringir las actualizaciones dinámicas del DNS y supervisar los cambios del DNS', 3, 'vulnerability de actualización dinámica del DNS'),
...
(268, 'Permite a los atacantes robar y abusar de las credenciales almacenadas', 'Almacenar credenciales de forma segura y usar solutiones de administración de credenciales seguras', 3, 'Almacenamiento de credenciales no confiables'),
...
--
-- Indices de la tabla `scope_vulnerabilities`
--
ALTER TABLE `scope_vulnerabilities`
  ADD PRIMARY KEY (`id`);
